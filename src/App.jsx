...
import LoanRegister from './components/LoanRegister.jsx'
...
function AppInner() {
  ...
  const { user, role, isAdmin, loading: authLoading, signIn, signOut } = useAuth()
  ...
  if (authLoading) {
    return (
      <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--navy-bg)'}}>
        <div style={{color:'white',fontSize:14,opacity:.7}}>Loading…</div>
      </div>
    )
  }
  ...
  function openDeleteVendor(v) { setDeleteTarget(v) }
  ...
  async function saveVendor() {
    ...
    if (editingVendor) {
      const { error } = await supabase.from('vendors').update({
        name: vName.trim(), vendor_type: vType, branch: vBranch||null, notes: vNotes||null,
      }).eq('id', editingVendor.id)
      err = error
    } else {
      const maxId = data.vendors.reduce((m,v) => Math.max(m, parseInt(v.vendor_id.replace('V',''))||0), 0)
      const { error } = await supabase.from('vendors').insert({
        vendor_id: 'V'+(maxId+1), name: vName.trim(),
        vendor_type: vType, branch: vBranch||null, notes: vNotes||null,
      })
      err = error
    }
    setSaving(false)
  ...
  async function confirmDelete() {
    ...
    const { count } = await supabase.from('vendor_transactions')
      .select('*',{count:'exact',head:true}).eq('vendor_id',deleteTarget.id)
    if (count > 0) {
    ...
    <>
      ...
      <header className="app-header">
        ...
        <div className="header-right">
          {error && <span style={{fontSize:10,color:'#fca5a5',maxWidth:140,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>⚠ {error}</span>}
          {/* Role badge */}
          <div className="auth-role-badge">
            {isAdmin ? '🔑 Admin' : '📋 Accountant'}
          </div>
          <div className="date-badge">{today}</div>
          <button className="dark-btn" onClick={()=>setDark(d=>!d)}>{dark?'☀️':'🌙'}</button>
          {/* Sign out */}
          <button className="dark-btn" onClick={signOut} title="Sign out" style={{fontSize:12}}>⏏</button>
        </div>
      </header>
